...
2. **Project Screen Validation**  
   ...
   - From a pipeline, run `npm run validate:screens -- <screenRoot> <workspaceRoot>` to execute the HTML scan inside the VS Code Extension Host without running the whole extension test suite. You can also set `SCREEN_VALIDATION_ROOT`, `SCREEN_VALIDATION_WORKSPACE_ROOT`, and `VSCODE_EXECUTABLE_PATH` when arguments are less convenient. Set `SCREEN_VALIDATION_FAIL_ON_DIAGNOSTICS=true` or pass `--fail-on-diagnostics` when diagnostics should fail the step.
   - For TypeScript coverage, run `npm run validate:screens:ts -- <screenRoot> <workspaceRoot>` to execute the backend-powered scan inside the VS Code Extension Host without running the whole extension test suite. You can also set `TS_SCREEN_VALIDATION_ROOT`, `TS_SCREEN_VALIDATION_WORKSPACE_ROOT`, and `VSCODE_EXECUTABLE_PATH`; set `TS_SCREEN_VALIDATION_FAIL_ON_DIAGNOSTICS=true` or pass `--fail-on-diagnostics` when diagnostics should fail the step.
