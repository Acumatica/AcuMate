...
| `acumate.validateTypeScriptScreens`| **Validate Screens (TypeScript)**    | Iterates through screen `.ts` files, runs the backend-powered `graphInfo` validator, and streams any warnings/errors to the **AcuMate Validation** output channel so you can review them without interrupting development. |
...
2. **Project Screen Validation**  
   ...
   - For TypeScript coverage, run **AcuMate: Validate Screens (TypeScript)** to traverse the same folder structure, execute `collectGraphInfoDiagnostics` for each screen `.ts`, and summarize backend metadata mismatches in the output channel (requires `acuMate.useBackend = true`).
