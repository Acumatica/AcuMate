...
2. **Project Screen Validation**  
   ...
   - For TypeScript coverage, run `npm run validate:screens:ts` to execute the backend-powered scan through the VS Code test runner. Override the root with `TS_SCREEN_VALIDATION_ROOT` when needed. The script logs summaries instead of failing on warnings.
